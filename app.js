...
const { startRecording, URLScreenshot, stopRecording, startBrowser, stopBrowser, URLScrollScreenshot } = require("./helpers/puppeteer");
    ...
    let content=[];
      if (r.URLs && r.URLs.length > 0) {
        for (let j = 0; j < r.URLs.length; j++) {
          ...
          console.log(r.id);
          //const screenshotPath = await URLScreenshot(page,url, `./${r.id}/${j}.jpg`);
          ...
          for (let k = 0; k < screenshotsPath.length; k++) {
            ...
            const text = await saveImageInfo({ path: s, prompt: prompt.SummarisePage, saveFilePath: `./${r.id}/${basename}.txt` });            
        ...
        let recorder =  await startRecording(page);
